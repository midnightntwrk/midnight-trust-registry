...
- For the first DID-backed integration slice, consume the official published `@midnight-ntwrk/midnight-did`, `@midnight-ntwrk/midnight-did-domain`, and `@midnight-ntwrk/midnight-did-contract` packages from npm instead of depending on sibling-repo paths or copied source trees at runtime.
