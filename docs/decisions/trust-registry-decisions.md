...
- For the first DID-backed integration slice, consume the official `@midnight-ntwrk/midnight-did`, `@midnight-ntwrk/midnight-did-domain`, and `@midnight-ntwrk/midnight-did-contract` packages from packed tarballs under `libs/` instead of depending on sibling-repo paths or copied source trees at runtime.
...
- For the first VC-backed integration slice, consume the official `@midnight-ntwrk/midnight-did-credentials` and `@midnight-ntwrk/midnight-did-credentials-status-registry` packages from packed tarballs under `tooling/vendor/` instead of modeling VC status evidence locally inside TR.
...
- Keep identity package refresh outside this repository: the root `midnight-identity-workspace` sync script builds tarballs in the owning DID/VC repositories and copies only `.tgz` artifacts into TR.
