...
import { JUBJUB_SUBGROUP_ORDER, modJubjubSubgroupOrder, } from "@midnight-ntwrk/midnight-did-credentials";
...
const bigintToBytes = (value, widthBytes) => {
    if (value < 0n) {
    ...
    if (remaining !== 0n) {
        throw new Error(`Cannot encode bigint into ${widthBytes} bytes without truncation`);
    }
    return encoded;
};
const assertValidAuthorityAttestedSigningInputs = ({ signer, createdAt, }) => {
    if (signer.secretKey <= 0n || signer.secretKey >= JUBJUB_SUBGROUP_ORDER) {
        throw new Error("Authority-attested status proof signer secret key must be in (0, JUBJUB_SUBGROUP_ORDER)");
    }
    if (createdAt < 0n) {
...
export const deriveAuthorityAttestedStatusProofNonceScalar = ({ statement, signer, createdAt, }) => {
    ...
    while (true) {
        const digest = createHash("sha256")
            ...
            .digest();
        const scalar = modJubjubSubgroupOrder(bytesToBigInt(new Uint8Array(digest)));
        if (scalar !== 0n) {
            return scalar;
        }
        attempt += 1n;
...
export const buildAuthorityAttestedStatusStatement = ({ request, statusHandleCommitment, expiresAt, }) => {
    const statement = {
 ...
 */
export const unsafeSignAuthorityAttestedStatusProofWithNonceScalar = ({ statement, signer, createdAt, nonceScalar, }) => {
    assertValidAuthorityAttestedSigningInputs({ signer, createdAt });
    if (nonceScalar <= 0n || nonceScalar >= JUBJUB_SUBGROUP_ORDER) {
        throw new Error("Authority-attested status proof nonce scalar must be in [1, JUBJUB_SUBGROUP_ORDER)");
    }
    const bodyRoot = pureCircuits.authorityAttestedStatusStatementRoot(statement);
    ...
    const challenge = pureCircuits.statusAttestationProofChallenge(bodyRoot, provisionalProof);
    const proof = {
        ...provisionalProof,
        signature: {
...
export const signAuthorityAttestedStatusProof = ({ statement, signer, createdAt, }) => unsafeSignAuthorityAttestedStatusProofWithNonceScalar({
    statement,
    signer,
    createdAt,
...
export const buildAuthorityAttestedStatusProofProtocol = ({ request, attestation, }) => {
    const protocol = {
        request,
        attestation,
    };
...
//# sourceMappingURL=attestation-builder.js.map