...
const serializeDigest = (digest) => Buffer.concat(digest.map((part) => bigintTo32Be(part)));
const hashToScalar = (input) => bufferToBigint(createHash("sha256").update(input).digest()) % JUBJUB_ORDER;
...
export const encodeJubjubSignature = (signature) => Buffer.concat([
...
export const signJubjubDigestFromSeed = (seedBytes, digest) => {
    ...
    return signJubjubDigest(seedBytesToJubjubSecretScalar(normalizedSeed), digest, Buffer.concat([
