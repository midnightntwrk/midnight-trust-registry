...
import { createMidnightDIDString, MidnightDIDSchema, MidnightNetwork, parseContractAddress, parseMidnightDID, parseMidnightDIDString, } from "../midnight";
...
describe("Midnight utilities", () => {
    ...
    it("maps auxiliary networks in Midnight DIDs", () => {
        const devnetDid = createMidnightDIDString(sampleAddress, MidnightNetwork.DevNet);
        const testnetDid = createMidnightDIDString(sampleAddress, MidnightNetwork.Testnet);
        expect(parseMidnightDID(parseMidnightDIDString(devnetDid)).network).toBe(MidnightNetwork.DevNet);
        expect(parseMidnightDID(parseMidnightDIDString(testnetDid)).network).toBe(MidnightNetwork.Testnet);
    });
    it("maps preview and preprod networks in Midnight DIDs", () => {
        const previewDid = createMidnightDIDString(sampleAddress, MidnightNetwork.Preview);
        const preprodDid = createMidnightDIDString(sampleAddress, MidnightNetwork.Preprod);
    ...
    it("validates schema helper", () => {
        const did = createMidnightDIDString(sampleAddress, MidnightNetwork.Undeployed);
