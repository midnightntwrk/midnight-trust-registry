import { createService, createVerificationMethod, CurveType, KeyType, VerificationMethodType, } from "@midnight-ntwrk/midnight-did-domain";
import { describe, expect, it } from "vitest";
import { MidnightDIDSchema } from "../midnight";
import { createMidnightDIDDocument, parseMidnightDIDDocument, } from "../midnight-did-document";
const exampleMidnightDid = MidnightDIDSchema.parse("did:midnight:testnet:c569622e7f33d2d020ba1cae242e6077268941327846d62d8cbf0cc923ae41f6");
...
describe("Midnight DID Document", () => {
    describe("createMidnightDIDDocument", () => {
        ...
        it("accepts Ed25519 (OKP) verification methods", () => {
            ...
            expect(doc.verificationMethod?.[0].publicKeyJwk.crv).toBe(CurveType.Ed25519);
        });
        it("accepts JubJub (EC) verification methods", () => {
            ...
            expect(doc.verificationMethod?.[0].publicKeyJwk.crv).toBe(CurveType.Jubjub);
        });
    ...
    describe("parseMidnightDIDDocument", () => {
        ...
        it("rejects document with string @context", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/@context must be an array/);
        });
        it("rejects document with only one @context entry", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/@context must contain at least 2 entries/);
        });
        it("rejects document with wrong first @context entry", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/First @context entry must be/);
        });
        it("rejects document with wrong second @context entry", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/Second @context entry must be/);
        });
        it("rejects document with non-Midnight DID", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/must be a valid Midnight DID/);
        });
        it("rejects document where controller does not equal id", () => {
            ...
            expect(() => parseMidnightDIDDocument(input)).toThrow(/controller must equal DID subject/);
        });
        ...
        it("accepts verification method with DID URL containing fragment", () => {
            ...
            const doc = parseMidnightDIDDocument(input);
            expect(doc.verificationMethod?.[0].id).toBe(`${exampleMidnightDid}#key-1`);
        });
...
//# sourceMappingURL=midnight-did-document.test.js.map