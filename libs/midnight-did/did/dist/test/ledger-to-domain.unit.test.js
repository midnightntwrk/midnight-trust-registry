...
describe("LedgerToDomain (unit, mocked managed runtime)", () => {
    ...
    beforeEach(() => {
        ...
        const verificationMethods = makeIterablePairs([
            [
                "key-1",
                {
        ...
        const services = makeIterablePairs([
            [
                "svc-1",
                {
                    id: "svc-1",
        ...
        stubLedger = {
            ...
            deactivated: false,
            operationCount: 3n,
            alsoKnownAs: makeIterable(["did:alias:one"]),
            verificationMethods,
    ...
    it("toJSON flattens ledger to plain JSON with arrays", () => {
        const json = LedgerToDomain.toJSON(stubLedger);
    ...
    it("ledgerStateToDIDDocument builds DID Document and assigns alsoKnownAs when present", () => {
        ...
        const normalizedServices = Array.from(stubLedger.services, ([, service]) => LedgerToDomain.service(service));
        expect(normalizedServices[0].id).toBe("#svc-1");
        expect(normalizedServices[1].id).toBe("#svc-2");
        let doc;
        try {
            doc = LedgerToDomain.ledgerStateToDIDDocument(stubLedger, MidnightNetwork.DevNet, addr);
        }
        catch (error) {
        ...
        expect("y" in (doc.verificationMethod?.[0]?.publicKeyJwk ?? {})).toBe(false);
    });
    it("ledgerStateToDIDDocument rejects relations to missing verification methods", () => {
        const addr = parseContractAddress("0".repeat(64));
        stubLedger.authenticationRelation = makeIterable(["missing-key"]);
        expect(() => LedgerToDomain.ledgerStateToDIDDocument(stubLedger, MidnightNetwork.DevNet, addr)).toThrow(/references missing verification method/);
    });
