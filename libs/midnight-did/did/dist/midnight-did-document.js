import { CurveType, DIDDocumentSchema, KeyType, VerificationMethodType, } from "@midnight-ntwrk/midnight-did-domain";
...
const MidnightVerificationMethodSchema = z
    ...
    .check(z.refine((vm) => vm.type === VerificationMethodType.JsonWebKey, "Midnight DID only supports JsonWebKey verification method type"), z.refine((vm) => {
    const kty = vm.publicKeyJwk.kty;
    return kty === KeyType.OKP || kty === KeyType.EC;
}, "Midnight DID only supports OKP (Ed25519) or EC (Jubjub/P-256) key types"), z.refine((vm) => {
    ...
    return false;
}, "OKP keys must use Ed25519 curve; EC keys must use Jubjub or P-256 curve"), z.refine((vm) => {
...
export const MidnightDIDDocumentSchema = DIDDocumentSchema.check(z.refine((doc) => Array.isArray(doc["@context"]), "@context must be an array for Midnight DID Documents"), z.refine((doc) => Array.isArray(doc["@context"]) && doc["@context"].length >= 2, "@context must contain at least 2 entries for Midnight DID Documents"), z.refine((doc) => Array.isArray(doc["@context"]) &&
    doc["@context"][0] === REQUIRED_CONTEXTS[0], `First @context entry must be '${REQUIRED_CONTEXTS[0]}'`), z.refine((doc) => Array.isArray(doc["@context"]) &&
    doc["@context"][1] === REQUIRED_CONTEXTS[1], `Second @context entry must be '${REQUIRED_CONTEXTS[1]}'`), z.refine((doc) => MidnightDIDSchema.safeParse(doc.id).success, "id must be a valid Midnight DID (did:midnight:<network>:<identifier>)"), z.refine((doc) => {
    // Controller must equal subject (single-controller model)
    if (!doc.controller)
        return true; // Optional, but if present must match
    if (typeof doc.controller === "string") {
        return doc.controller === doc.id;
    }
    // If array, must have exactly one entry equal to id
    return (Array.isArray(doc.controller) &&
        doc.controller.length === 1 &&
        doc.controller[0] === doc.id);
}, "controller must equal DID subject for Midnight DID (single-controller model)"), z.refine((doc) => {
    // All verification methods must be JsonWebKey and use supported key types
    if (!doc.verificationMethod)
        return true;
    return doc.verificationMethod.every((vm) => MidnightVerificationMethodSchema.safeParse(vm).success);
...
export function createMidnightDIDDocument(params) {
    ...
    const parsed = MidnightDIDDocumentSchema.parse(doc);
    return {
 ...
 */
export const parseMidnightDIDDocument = (input) => {
    const parsed = MidnightDIDDocumentSchema.parse(input);
    const id = MidnightDIDSchema.parse(parsed.id);
    const controller = parsed.controller == null
        ? parsed.controller
        : Array.isArray(parsed.controller)
            ? parsed.controller.map((value) => MidnightDIDSchema.parse(value))
            : MidnightDIDSchema.parse(parsed.controller);
    return {
        "@context": parsed["@context"],
        id,
        alsoKnownAs: parsed.alsoKnownAs ?? null,
...
//# sourceMappingURL=midnight-did-document.js.map