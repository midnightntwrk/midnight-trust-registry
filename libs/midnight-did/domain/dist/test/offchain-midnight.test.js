...
import { createOffchainMidnightDIDStringFromState, createPortableOffchainMidnightDIDUrl, decodeOffchainMidnightDIDState, encodeOffchainMidnightDIDState, offchainStateToDidDocument, parsePortableOffchainMidnightDIDUrl, } from "../offchain-midnight.js";
const sampleState = {
    version: 1,
...
describe("offchain Midnight DID helpers", () => {
    ...
    it("creates and parses a portable offchain Midnight DID URL", () => {
        ...
        expect(parsed.did).toBe(createOffchainMidnightDIDStringFromState(sampleState));
    ...
    it("rejects a portable DID URL when the state payload is tampered", () => {
        ...
        expect(() => parsePortableOffchainMidnightDIDUrl(tampered)).toThrow(/state does not match the DID state hash/);
    });
    ...
    it("round-trips Ed25519 and P-256 verification methods", () => {
        ...
        expect(decodeOffchainMidnightDIDState(encoded)).toEqual(stateWithMultipleKeys);
    });
    it("rejects malformed encoded state payloads", () => {
        const encoded = encodeOffchainMidnightDIDState(sampleState);
        const bytes = Buffer.from(encoded.payload.replaceAll("-", "+").replaceAll("_", "/") +
            "=".repeat((4 - (encoded.payload.length % 4)) % 4), "base64");
        const wrongMagic = Buffer.from(bytes);
        wrongMagic[0] = 0x00;
        expect(() => decodeOffchainMidnightDIDState({
            ...encoded,
            payload: wrongMagic
                .toString("base64")
                .replaceAll("+", "-")
        ...
        expect(() => decodeOffchainMidnightDIDState({
            ...
            payload: Buffer.from(truncated)
                ...
                .replaceAll("/", "_")
                .replace(/=+$/u, ""),
        })).toThrow(/chunk exceeds payload length|trailing bytes|shorter than the header|ended before uint32 field/);
        expect(() => decodeOffchainMidnightDIDState({
            ...encoded,
            payload: "!",
    ...
    it("rejects state shapes beyond the prototype bounds", () => {
        expect(() => encodeOffchainMidnightDIDState({
            ...sampleState,
        ...
        expect(() => encodeOffchainMidnightDIDState({
            ...sampleState,
            verificationMethod: new Array(5).fill(sampleState.verificationMethod[0]),
        })).toThrow(/verificationMethod must contain at most 4 entries/);
    });
});
//# sourceMappingURL=offchain-midnight.test.js.map