...
import { DIDDocumentMetadataSchema, KnownDIDMediaTypesSchema, parseDID, parseDIDDocument, parseDIDKeyID, parseDIDResolutionResult, parseDIDURL, parseService, parseVerificationMethod, parseVerificationMethodRelation, parseVerificationMethodType, VerificationMethodType, } from "../did-document";
import { exampleDid, exampleDidUrl, exampleMethodId, exampleResolutionPayload, exampleServiceInput, exampleServiceObjectInput, exampleVerificationMethodInput, invalidDidStrings, } from "./fixtures/did";
describe("DID parsing utilities", () => {
    ...
    it("parses and validates DID Key IDs", () => {
        ...
        expect(parseDIDKeyID("#rel-key")).toBe("#rel-key");
        expect(() => parseDIDKeyID(exampleDid)).toThrow();
        expect(() => parseDIDKeyID("did:example:abc#key?bad")).toThrow();
        expect(() => parseDIDKeyID("not/allowed/path")).toThrow();
    });
    it("accepts all known DID media types", () => {
        const entries = Object.values(KnownDIDMediaTypesSchema.def.entries);
    ...
    it("parses verification method and service helpers", () => {
        expect(parseVerificationMethod(exampleVerificationMethodInput)).toEqual(exampleVerificationMethodInput);
        expect(parseService(exampleServiceInput)).toEqual(exampleServiceInput);
        expect(parseService(exampleServiceObjectInput)).toEqual(exampleServiceObjectInput);
        expect(parseVerificationMethodType(VerificationMethodType.JsonWebKey)).toBe(VerificationMethodType.JsonWebKey);
        expect(parseVerificationMethodRelation("Authentication")).toBe("Authentication");
    });
    ...
    it("rejects metadata timestamps with sub-second precision", () => {
        expect(() => DIDDocumentMetadataSchema.parse({
            created: "2024-01-01T00:00:00.123Z",
        })).toThrow(/Invalid input|datetime/);
    });
});
//# sourceMappingURL=did-document.parse.test.js.map